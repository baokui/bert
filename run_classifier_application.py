...
flags.DEFINE_integer("nb_classes", 2,
                     "nb of classses")
flags.DEFINE_integer("nb_examples", -1,
                     "nb of examples")
...
class myProcessor(DataProcessor):
  ...
  def get_labels(self,nb_classes):
    """See base class."""
    return [str(i) for i in range(nb_classes)]
...
def netConfig():
    ...
    for t in tvars:
        print(t)
class bert_cls:
    def __init__(self,path_model,num_labels,max_seq_length = 128):
        ...
        bert_config_file = os.path.join(path_model,'bert_config.json')
        ...
        (self.loss, self.per_example_loss, self.logits, self.probabilities) = create_model(bert_config, is_training, self.input_ids, self.input_mask, self.segment_ids,
                 self.labels, num_labels, use_one_hot_embeddings=False)
