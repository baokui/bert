...
def main(_):
    ...
    if FLAGS.do_eval:
        eval_examples = processor.get_dev_examples(FLAGS.data_dir, idx_label=idx_label)
    ...
    if FLAGS.do_predict:
        predict_examples = processor.get_test_examples(FLAGS.data_dir, idx_label=idx_label)
...
class bert_cls:
    ...
    def __init__(self, path_vocab, path_config, path_model, init_checkpoint=None,
        ...
        is_training = False
        (loss, per_example_loss, logits, self.probabilities) = create_model(bert_config, is_training, self.input_ids,
                                                                            self.input_mask, self.segment_ids,
...
def test():
    ...
    thr0 = [i*0.01 for i in range(100)]
