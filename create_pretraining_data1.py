...
class TrainingInstance(object):
    ...
    def __init__(self, tokens, segment_ids, masked_lm_positions, masked_lm_labels,
                 is_random_next,tipToken,preToken):
        self.tokens = tokens
        self.segment_ids = segment_ids
        self.is_random_next = is_random_next
        self.masked_lm_positions = masked_lm_positions
        self.masked_lm_labels = masked_lm_labels
        self.tipToken = tipToken
    ...
    def __str__(self):
        ...
        s += "tipToken: %s\n" % (" ".join([str(x) for x in self.tipToken]))
        s += "preToken: %s\n" % (" ".join([str(x) for x in self.preToken]))
        s += "\n"
        return s
    def __repr__(self):
        return self.__str__()
...
def write_instance_to_example_files(instances, tokenizer, max_seq_length,
    ...
    for (inst_index, instance) in enumerate(instances):
        ...
        tip_ids = tokenizer.convert_tokens_to_ids(instance.tipToken)
        pre_id = tokenizer.convert_tokens_to_ids([instance.preToken])
        ...
        features["tip_ids"] = create_int_feature(tip_ids)
        features["pre_id"] = create_int_feature(pre_id)
...
def create_instances_from_document(
        all_documents, document_index, max_seq_length, short_seq_prob,
        masked_lm_prob, max_predictions_per_seq, vocab_words, rng, winSize = 2):
